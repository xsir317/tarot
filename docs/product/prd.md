...
| F-010 | 匿名用户额度 | P0 | 待开发 | 仅使用 Browser Storage 简单标记，暂不实现复杂防刷策略 |
| F-011 | 用户注册 | P0 | 待开发 | 手机号/邮箱验证码登录/注册；MVP阶段验证码通过API直接返回，不发送真实短信/邮件 |
| F-012 | 按次付费 | P0 | 待开发 | 完整支付状态流转；MVP阶段Mock支付网关交互 |
| F-013 | 包月订阅 | P0 | 待开发 | 完整订阅状态流转；MVP阶段Mock支付网关交互 |
...
| F-017 | 占卜打赏 | P1 | 待开发 | 针对特定占卜结果进行打赏（类似还愿），支持从历史记录发起 |
...
### 安全
- 用户密码 bcrypt 加密 (注: 如采用纯验证码登录，则无需密码)
- JWT token 有效期合理设置
...
| 设备指纹可靠性 | 低 | MVP阶段仅使用简单标记，接受一定程度的滥用风险 |
| 支付渠道限制 | 低 | 使用 Stripe，支持多地区支付 |

### 外部依赖
- OpenAI/Anthropic API
- Stripe API (MVP阶段 Mock)
- 邮件服务（MVP阶段 Mock）
- 短信服务（MVP阶段 Mock）
